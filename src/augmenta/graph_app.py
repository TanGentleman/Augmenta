...
def agent_node(state: GraphState) -> GraphState:
    """
    This node is responsible for handling the agent's decision making and conversation flow.
    ...
    print("Agent exit condition fulfilled!") if is_done else None
    return {
        "keys": new_keys,
        "mutation_count": state["mutation_count"] + 1,
        "is_done": is_done
...
def decide_from_chatbot(state: GraphState) -> Literal["end_node", "human_node", "tools_node"]:
    """
    This function is responsible for deciding whether the next node should be the agent or the human node.

    Args:
        state (GraphState): _description_

    Returns:
        Literal["agent", "human"]: _description_
    """
    if state["is_done"]:
        return "end_node"
    state_dict = state["keys"]
...
def decide_from_human(state: GraphState) -> Literal["agent_node", "apply_command_node"]:
    """
    This function is responsible for deciding whether the next node should be the agent node or the apply_command node.

    Args:
        state (GraphState): _description_

    Returns:
        Literal["agent_node", "apply_command_node"]: _description_
    """
    state_dict = state["keys"]
...
def decide_from_tools(state: GraphState) -> Literal["agent_node", "generate_node"]:
    """
...
# workflow.add_edge("human_node", "agent_node")
...
workflow.add_edge("apply_command_node", "agent_node")
