...
async def process_email(
    ...
    fetch_params = FetchParams(source=str(email_path), method='get_email_content')

    plan_params = PlanParams(max_revisions=max_revisions)

    extra_params = ExtraParams(system_message=system_message)

    await plan_email(fetch_params=fetch_params, plan_params=plan_params, extra_params=extra_params)
