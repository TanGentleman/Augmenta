...
def get_task(
    ...
    if task_name:
        ...
        task = next(
            (task for task in task_dict.values() if task["status"] == status), None
        )
    else:
...
def save_stashed_tasks(task_dict: dict[str, Task]) -> list[str]:
    ...
    stashed_tasks = []
    for task_name, task in task_dict.items():
        if task["status"] == Status.IN_PROGRESS and is_stash_action_next(
            task["actions"]
        ):
        ...
        case TaskType.CHAT:
            ...
            if not config.chat_settings.disable_system_message:
                insert_system_message(
                    task_state["messages"], config.chat_settings.system_message
                )
